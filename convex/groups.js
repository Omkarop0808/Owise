...
export const getGroupExpenses = query({
  ...
  handler: async (ctx, { groupId }) => {
    ...
    // format: { userId1: balance1=0, userId2: balance2=0 ,...}
    const totals = Object.fromEntries(ids.map((id) => [id, 0]));
    // for 3 users (user1,user2,user3)
    // ledger ={
    //      "user1":{"user2":0,"user3":0}, how much 2 & 3 going to pay 
    //     "user2":{"user1":0,"user3":0},
    //     "user3":{"user1":0,"user2":0}
    // }
    // pair‑wise ledger  debtor -> creditor -> amount//2d ledger
    const ledger = {};
    // initailize ledger
    ids.forEach((a) => {
      ledger[a] = {};
      ids.forEach((b) => {
       ...
       /* ----------  apply expenses ---------- */
    // example:
    // expense1: user1 paid $40,split equally among all 3 user which is ($20 each)
    ...
    // }
    // -this mean user2 owes $20 to user1 and user3 owes $20 to user1
    for (const exp of expenses) {
      ...
      for (const split of exp.splits) {
        ...
        totals[payer] += amt;//payer gain amt
        totals[debtor] -= amt;//payer lose amt since into debit
    ...
    /* ----------  apply settlements ---------- */
    // Example
    // -settlements:user2 paid $10  to user1
    ...
    // "user3":{"user1":20,"user2":0}
    // }
    // -this mean user2 now owes $10 to user1 and user3 owes $20 to user1
    for (const s of settlements) {
        // increase payer balance and decrease the receiver balance
    ...
    /* ----------  net the pair‑wise ledger ---------- */
        // simplifying the ledger (debt simplificaion)
    // example 1
    // -intial ledger :
    // -user1 owes user2 $10
    // -user2 owes user3 $15
    ...
    // -user2 owes user3 $15
    // user3 owes user1 $0
    // this reduces circular debt pattern
